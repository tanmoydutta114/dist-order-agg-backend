...
async function processOrderWithItems(orderId: number) {
  await getSQLClient().transaction().execute(async (trx) => {
    ...
    console.log(
      `🔄 Processing order: ${order.quantity}x ${order.vendor_product_id}`
    );
    ...
    const availableProducts = await trx
      ...
      .where("products.vendor_product_id", "=", order.vendor_product_id)
      .where("products.is_active", "=", true)
      .where("products.stock_quantity", ">", 0)
      .orderBy("products.price", "asc") // Allocate from cheapest vendor first
      .forUpdate() // Lock all relevant product rows
      .execute();

    if (availableProducts.length === 0) {
      throw new Error(
        `No stock available for product ${order.vendor_product_id}`
      );
    }

    // 3. Smart allocation across vendors
    const allocations = await allocateStock(availableProducts, order.quantity);

    if (allocations.totalAllocated < order.quantity) {
      throw new Error(
        `Insufficient stock: need ${order.quantity}, can allocate ${allocations.totalAllocated}`
      );
    }

    // 4. Reserve stock and create order items
    let totalAmount = 0;
    ...
    for (const allocation of allocations.items) {
      ...
      console.log(
        `  🎯 Reserved ${allocation.quantity}x from ${allocation.vendorId} at $${allocation.price} each`
      );
    }
...
async function allocateStock(
  ...
  for (const product of availableProducts) {
    ...
    console.log(
      `  📋 Plan: ${allocateQuantity}x from ${product.vendor_id} (${product.vendor_name}) at $${product.price}`
    );
  }
